...
def launch_transactions(full_node_backend_address, trust_score_backend_address, source_seed, source_address_index,
  ...
  for i in (range(destination_addresses_length)):
    ...
    currency_hash = currency_hash.strip()
    if currency_hash == 'ae2b227ab7e614b8734be1f03d1532e66bf6caf76accc02ca4da6e28':
      # Native token case
      ...
      # CMD token case
      full_node_fee, input_base_transaction_coti, input_base_transaction_cmd, instant_time, instant_time1, network_fee_data, private_key, \
          public_key, receiver_base_transaction, transaction_description, transaction_hash, \
          transaction_trust_score_data = \
          cmd_tx.call_apis_to_prepare_a_CMD_tx(source_address_public_key_crc, destination_address, amount, "ae2b227ab7e614b8734be1f03d1532e66bf6caf76accc02ca4da6e28", currency_hash,
                                               source_seed, address_private, transaction_description, full_node_backend_address,
                                               trust_score_backend_address, fee_included)

      transaction_creation_result = cmd_tx.create_CMD_transaction(full_node_backend_address, full_node_fee,
                                                                  input_base_transaction_coti, input_base_transaction_cmd,
                                                                  instant_time,
                                                                  instant_time1,
                                                                  network_fee_data, private_key, public_key,
                                                                  receiver_base_transaction,
  ...
  modules_in_out_logger.info("\t++++> launch_transactions took: " + str(int(datetime.datetime.now().timestamp()) - launch_transactions_start_time) + " sec <++++")
...
def read_env_file(env_filepath):
  mapping = dict.fromkeys(range(32))
  env_details = {}
